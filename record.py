...
class MainWindow():
    ...
    def create_main_panel(self):
        ...
        self.outputDirStrVar.set("samples/" + datetime.now().strftime('%Y_%m_%d_%H_%M_%S'))
    ...
    def init_plot(self):
        ...
        self.plotData = [[0] * (21)] * self.plotMem # mem storage for plot
    ...
    def save_data(self):
        # self.frameData.append(FrameData(fileName = self.outputDir+'/'+'img_'+str(self.t)+'.jpeg', controllerState = self.controller_data, imageData = self.img))
        QUEUE.put(FrameData(fileName = self.outputDir+'/'+'img_'+str(self.t)+'.jpeg', controllerState = self.controller_data, imageData = self.img))
        """
        image_file = self.outputDir+'/'+'img_'+str(self.t)+'.jpeg'
    ...
    def on_btn_record(self):
        ...
        if self.recording:
            ...
            fileName = self.outputDir+'/'+'data.csv'

            # Create processes to pull from queue
            self.process = Process(target=save, args=(QUEUE, fileName))
            self.processTwo = Process(target=save, args=(QUEUE, fileName))
            self.process.start()
            self.processTwo.start()
        else:
            self.record_button["text"] = "Record"
            self.rate = self.idle_rate
            # for data in self.frameData:
            #     data.save(self.outfile)
            # self.outfile.close()
            while not QUEUE.empty():
                print("Queue size remaining: ", QUEUE.qsize())
                time.sleep(0.5)
            print("Queue is empty")

            # Destroy the processes once the queue is empty
            self.process.terminate()
            self.processTwo.terminate()
