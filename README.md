...
Okay, so essentially, what we're trying to do is learn a *Policy* that most closely matches the *expert policy* based on the *demonstrations alone*. As it turns out, this can be accomplished using *Behavior Cloning*. Essentially, the goal is to train a model using supervised learning, where given a state (the frame at time X, and optionally the controller state at time X), the model must learn the policy, which is defined as its output action (the controller state at time X+1). We'll go with this type of formulation as its pretty much our main choice given that we *don't have any rewards to go on*. While I could probably create a reward function for Halo, I'd rather avoid reward tuning for now.
